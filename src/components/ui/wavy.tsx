...
/**
 * WavyBackground — shader WebGL de ondas fluidas (fractal noise + domain warping).
 * Adaptado de 21st.dev para LADO ₿ (paleta dourada).
 *
 * Serve como fundo ideal para Liquid Glass porque tem textura rica e variação
 * de cor suficiente pro feDisplacementMap SVG distorcer visivelmente.
 */
...
// Paleta LADO ₿ — do preto profundo até dourado Bitcoin brilhante
...
function buildFragmentShader(): string {
  ...
  const colorArraySrc = goldColors
...
vec3 goldColors[NUM_COLORS] = vec3[](
  ${colorArraySrc}
);
...
void main() {
  ...
  float f = fract(idx);
  vec3 colLow = goldColors[iLow];
  vec3 colHigh = goldColors[iHigh];
